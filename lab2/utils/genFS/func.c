...
int cp (const char *driver, const char *srcFilePath, const char *destFilePath) {
    ...
    // read fs info
    if (readSuperBlock(file, &superBlock) == -1) {
        printf("Failed to load superBlock.\n");
        fclose(file);
        fclose(srcfile);
        return -1;
    ...
    if (stringChrR(destFilePath, '/', &pos) == -1) {
		printf("Incorrect destination file path.\n");
        fclose(file);
		fclose(srcfile);
        return -1;
	}
    char tmp = *((char*)destFilePath + pos + 1);	// only keep father dirEntry
    *((char*)destFilePath + pos + 1) = 0;
    ret = readInode(file, &superBlock, &fatherInode, &fatherInodeOffset, destFilePath);
    *((char*)destFilePath + pos + 1) = tmp;
    if (ret == -1) {
    	printf("Failed to read father inode.\n");
        fclose(file);
        fclose(srcfile);
    ...
    ret = allocInode(file, &superBlock, &fatherInode, fatherInodeOffset,
        &destInode, &destInodeOffset, destFilePath + pos + 1, REGULAR_TYPE);
    if (ret == -1) {
        printf("Failed to allocate inode.\n");
        fclose(file);
        fclose(srcfile);
