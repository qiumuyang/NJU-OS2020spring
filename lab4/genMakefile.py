...
built_in = ['utils/genFS', 'bootloader', 'kernel', 'app', 'utils', 'lib']
...
'''
    ...
    head += '\n\t@./utils/genFS/genFS app/uMain.elf ' + ' '.join(fullnames)
    mid = '''
...
def read_apps():
    ...
    data = [[name, insideName[i], name.split(
        "/")[0], name.split("/")[1]] for i, name in enumerate(outsideName)]
    return data
...
def add_apps():
    data = []
    folders = [item for item in os.listdir('.') if os.path.isdir(item) and item not in built_in]
    for folder in folders:
        files = os.listdir(folder)
        elf_name = folder
        data.append([os.path.join(folder, elf_name+'.elf'), f'/usr/{elf_name}', folder, elf_name+'.elf'])
        if 'Makefile' not in files:
            write_AppMakefile(folder, elf_name)
    return data
