			...
			case 1: // '%' is on top of stack
				switch(format[i]){
					...
					case 'd':
						...
						count=dec2Str(decimal, buffer, MAX_BUFFER_SIZE, count, font, bg);
						break; // decimal
      					case 'x':
						state = 0;
						index+=4;
						hexadecimal=*(uint32_t*)(paraList+index);
						count=hex2Str(hexadecimal, buffer, MAX_BUFFER_SIZE, count, font, bg);
						break; // hexadecimal
					case 's':
						state = 0;
						index+=4;
						string=*(char**)(paraList+index);
						count=str2Str(string, buffer, MAX_BUFFER_SIZE, count, font, bg);
						break; // string
					case 'c':
						state = 0;
						index+=4;
...
uint32_t rand() {
    ...
    return (uint32_t)((RandNext/65536) % RAND_MAX);
}
