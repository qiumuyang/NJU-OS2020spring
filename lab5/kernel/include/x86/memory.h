...
struct File {
	...
	int flags;
	int f_count;		// for fd-share among father-child processes
	char name[64];
};
...
struct ProcessTable {
	...
	char name[32];
	int fd[MAX_FILE_NUM];
	uint8_t signal[MAX_SIG_NUM]; // pending signals
	int handler[MAX_SIG_NUM];    // signal handler
	struct ListHead blocked; // semaphore, device, file blocked on
	int cwd;	// current working directory, inode
	struct ListHead pcb;	// other process blocked on it
};
