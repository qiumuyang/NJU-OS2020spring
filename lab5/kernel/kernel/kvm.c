...
int loadElf(const char *filename, uint32_t physAddr, uint32_t *entry) {
	...
	for (i = 0; i < eh->phnum; i++) {
		...
		//putHex(physAddr + ph->vaddr + ph->memsz, '\n');
		ph++;
	}
	*entry = eh->entry;
	return 0;
...
void initProc() {
	...
	for (i = 0; i < MAX_PCB_NUM; i++) {
		...
		pcb[i].fd[STD_OUT] = STD_OUT;
		pcb[i].fd[STD_IN] = STD_IN;
		pcb[i].fd[STD_ERR] = STD_ERR;
		pcb[i].fd[SH_MEM] = SH_MEM;
		pcb[i].blocked.next = &(pcb[i].blocked);
		pcb[i].blocked.prev = &(pcb[i].blocked);
		pcb[i].pcb.next = &(pcb[i].pcb);
		pcb[i].pcb.prev = &(pcb[i].pcb);
	}
	// kernel process
	...
	pcb[0].pid = 0;
	pcb[0].ppid = 0;
	stringCpy("idle", pcb[0].name, 5);
	pcb[0].cwd = rootDirInode;
	initHeap(0x100000);
	// user process
