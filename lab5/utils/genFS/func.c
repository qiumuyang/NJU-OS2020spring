 ...
 *  Input: file, superblock, inode.
 *  No Error Checking.
 */
int freeBlock (FILE *file, SuperBlock *superBlock, Inode *inode) {
	int j = 0;
	int k = 0;
	int divider0 = superBlock->blockSize / 4;
    int bound0 = POINTER_NUM;
...
/*
 *  Free an Inode.
 *  Remove it from fatherInode, update:
 *  		1. superBlock avail[Block/Inode]Num
 * 			2. [Block/Inode]Bitmap 
 *			3. fatherInode
 *			4. destInode
 */
int freeInode(FILE *file, SuperBlock *superBlock, Inode *fatherInode, int fatherInodeOffset,
    ...
    if (i == fatherInode->blockCount) {
    	printf("Inode not found in fatherInode.\n");
    	return -1;
    }
    ...
    ret = writeBlock(file, superBlock, fatherInode, i, buffer);
    if (ret == -1) {
    	printf("Fail to write fatherInode's DirEntry");
        return -1;
    }
    
    // free destInode's Blocks
    freeBlock(file, superBlock, destInode);
...
int isDirEmpty(FILE *file, SuperBlock *superBlock, Inode *inode) {
	int i = 0;
    int j = 0;
    int ret = 0;
    DirEntry *dirEntry = NULL;
...
int rmdir (const char *driver, const char *destDirPath) {
    ...
    if (driver == NULL || destDirPath == NULL) {
        printf("driver == NULL || destDirPath == NULL.\n");
        return -1;
    }
    file = fopen(driver, "r+");
    if (file == NULL) {
    ...
    if (ret == -1) { 	// no '/' in destDirPath
        printf("Incorrect destination dir path.\n");
        fclose(file);
        return -1;
    }
    if (size == 0 && length == 1) {
    	printf("Unable to remove RootDir('/').\n");
        fclose(file);
        return -1;
    }
    if (strcmp("..", destDirPath + size + 1) == 0
     || strcmp(".", destDirPath + size + 1) == 0) {
    	printf("Unable to remove Dir(%s).\n", destDirPath + size + 1);
        fclose(file);
        return -1;
    }
    tmp = *((char*)destDirPath + size + 1);
    *((char*)destDirPath + size + 1) = 0;  // destDirPath is dir ended with '/'.
    ret = readInode(file, &superBlock, &fatherInode, &fatherInodeOffset, destDirPath);
    *((char*)destDirPath + size + 1) = tmp;
    if (ret == -1) {
        printf("Failed to read father inode.\n");
    ...
    if (destInode.type != DIRECTORY_TYPE) {
    	printf("Expecting type Directory but type %d was given.\n", destInode.type);
        fclose(file);
        return -1;
    }
    if (!isDirEmpty(file, &superBlock, &destInode)) {
    	printf("Dir %s is not empty.\n", destDirPath);
        fclose(file);
        return -1;
    }
    ret = freeInode(file, &superBlock, &fatherInode, fatherInodeOffset, &destInode, destInodeOffset, destDirPath + size + 1);
    if (ret == -1) {
...
int rm (const char *driver, const char *destFilePath) {
    ...
    if (destFilePath[length - 1] == '/') {
    	printf("Use rmdir to remove a dir.\n");
        fclose(file);
        return -1;
    }
    ...
    *((char*)destFilePath + size + 1) = tmp;
    ...
    ret = freeInode(file, &superBlock, &fatherInode, fatherInodeOffset, &destInode, destInodeOffset, destFilePath + size + 1);
    if (ret == -1) {
        printf("Failed to free inode.\n");
        fclose(file);
        return -1;
...
int touch (const char *driver, const char *destFilePath, int destFiletype) {
    FILE *file = NULL;
    char tmp = 0;
    int ret = 0;
    int size = 0;
    ...
    *((char*)destFilePath + size + 1) = tmp;
    ...
    ret = allocInode(file, &superBlock, &fatherInode, fatherInodeOffset,
        &destInode, &destInodeOffset, destFilePath + size + 1, destFiletype);
    if (ret == -1) {
        printf("Failed to allocate inode.\n");
        fclose(file);
        return -1;
